...
impl MissionTracker {
    ...
    fn retain_snapshot(&mut self, active_ids: impl Iterator<Item = u64>) {
...
fn mission_kind(event: &MissionEvent) -> MissionKind {
    if event.kill_count.unwrap_or(0) > 0 || mission_text(event).contains("massacre") {
    ...
    if event.count.unwrap_or(0) > 0
        ...
        .iter()
        .any(|needle| mission_text(event).contains(needle))
    {
