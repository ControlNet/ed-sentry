...
- CLI startup output now uses a reference-style three-line banner: `ED AFK Monitor v260421 by CMDR PSIPAB`, followed by `Journal folder`, basename-only `Journal file`, `Commander name`, `Config profile: Default`, `Starting... (Press Ctrl+C to stop)`, and the terminal-only Discord info line.
