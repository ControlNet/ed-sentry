...
- Avoid infinite `.cmd` loops in Windows process fixtures. Finite post-URL waits keep the child alive for provider ownership tests without risking a stuck GitHub Actions test step if Windows process-tree cleanup behaves differently.
