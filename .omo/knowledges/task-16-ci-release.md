...
- Locked artifact names are `ed-afk-dashboard-x86_64-unknown-linux-gnu.tar.gz` and `ed-afk-dashboard-x86_64-pc-windows-msvc.zip`.
- Workflows must not invoke `real_journal_replay -- --ignored` and must not reference `/home/ubuntu/Elite Dangerous`; those remain local-only verification paths.
