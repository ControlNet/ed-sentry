...
- Public parser API lives in `ed_afk_monitor::event`: `parse_journal_line(&str)` for line-delimited Journal input and `parse_journal_value(&serde_json::Value)` for already-decoded JSON.
