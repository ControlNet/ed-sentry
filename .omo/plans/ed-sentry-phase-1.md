# ed-sentry Phase 1: Rust CLI AFK Journal Monitor

## TL;DR
> **Summary**: Initialize an independent Rust CLI named `ed-sentry` that tails/replays Elite Dangerous Journal files, maintains AFK session state, renders terminal event/status output, and emits structured notifications through a Matrix-ready abstraction without implementing Matrix yet.
> **Deliverables**:
> - Rust crate/binary `ed-sentry` with clean module boundaries.
...
- Locked crate/package/binary/artifact prefix to `ed-sentry`.
...
- Exact valid examples:
  ```bash
  ed-sentry --journal "/home/ubuntu/Elite Dangerous"
  ed-sentry --journal "/home/ubuntu/Elite Dangerous" --poll-interval-ms 1000
  ed-sentry --debug --replay --set-file tests/fixtures/journal_combat_bounty.log --no-status-line
  ed-sentry --set-file tests/fixtures/journal_combat_bounty.log --replay --no-status-line
  ```
- Exact invalid examples:
  ```bash
  ed-sentry --replay --poll-interval-ms 1000
  ed-sentry --replay --journal "/home/ubuntu/Elite Dangerous"
  ```
...
- [x] 1. Initialize Rust project, toolchain, and baseline quality gates
  ...
  **Acceptance Criteria**:
  ...
  - [ ] `cargo run -- --version` exits `0` and stdout contains `ed-sentry`.
  ...
  **QA Scenarios**:
  ...
  Scenario: Fresh project builds metadata
    ...
    Expected: command exits 0; evidence file contains package name "ed-sentry"
...
- [x] 2. Define CLI and TOML config contract with tests
  ...
  **QA Scenarios**:
  ...
  Scenario: Malformed TOML returns runtime config error
    Tool: Bash
    Steps: printf '[monitor\n' > /tmp/opencode/ed-sentry-bad.toml; cargo run -- --config /tmp/opencode/ed-sentry-bad.toml watch > .omo/evidence/task-2-bad-config.txt 2>&1; test $? -eq 1
...
- [x] 15. Write README and config documentation for Phase 1 only
  ...
  **Acceptance Criteria**:
  ...
  - [ ] README includes exact artifact names `ed-sentry-x86_64-unknown-linux-gnu.tar.gz` and `ed-sentry-x86_64-pc-windows-msvc.zip`.
...
- [x] 16. Add GitHub Actions CI and release artifact workflows
  ...
  **What to do**: Add `.github/workflows/ci.yml` running on push/PR for `ubuntu-latest` and `windows-latest`: checkout, install stable Rust, cache cargo, run `cargo fmt --check`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo test --all`. Add `.github/workflows/release.yml` triggered by tags `v*`: build `--release` on native `ubuntu-latest` and `windows-latest`; package Linux binary as `ed-sentry-x86_64-unknown-linux-gnu.tar.gz`; package Windows `.exe` as `ed-sentry-x86_64-pc-windows-msvc.zip`; upload artifacts. Add tests or script checks to validate workflow YAML contains both runners and exact artifact names.
  ...
  **Acceptance Criteria**:
  ...
  - [ ] Workflow YAML contains exact artifact names `ed-sentry-x86_64-unknown-linux-gnu.tar.gz` and `ed-sentry-x86_64-pc-windows-msvc.zip`.
  ...
  **QA Scenarios**:
  ...
  Scenario: Workflow artifact names are exact
    Tool: Bash
    Steps: grep -R "ed-sentry-x86_64-unknown-linux-gnu.tar.gz\|ed-sentry-x86_64-pc-windows-msvc.zip" .github/workflows > .omo/evidence/task-16-artifacts.txt
...
## Success Criteria
- `ed-sentry` can replay sanitized fixtures and emit deterministic terminal event/status output.
- `ed-sentry watch --journal /home/ubuntu/Elite Dangerous --no-status-line` can preload and tail the selected latest Journal without modifying the folder.
