...
test("@tauri-window-chrome enables frameless drag and window controls", async () => {
  ...
  expect(shellSource).toContain('label: "Dashboard"')
...
test("@tauri-window-chrome visualizes titlebar drag hitmap", async ({ page }) => {
  ...
  await expect(page.locator("[data-titlebar-no-drag='workspace-tab']")).toHaveCount(5)
...
test("@tauri-window-chrome does not block window setup on desktop runtime startup", async () => {
  ...
  expect(desktopGuiSource).toContain("open_external_url")
  expect(desktopGuiSource).toContain("tauri_plugin_opener::init()")
