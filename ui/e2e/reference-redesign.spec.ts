...
test("@reference-redesign renders the reference-derived workspace tabs", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: /Telemetry Interface/u })).toBeVisible()
  ...
  await expect(page.getByRole("heading", { name: /Events Interface/u })).toBeVisible()
  ...
  await expect(page.getByRole("heading", { name: /Config Interface/u })).toBeVisible()
